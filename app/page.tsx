...
export default function Page() {
  ...
  const send = async () => {
    ...
    try {
      ...
      // Pastikan hanya string output yang dikirim ke message, bukan objek JSON
      setMessages((m) => [
        ...m,
        {
          id: getUserId(),
                    ...
                    <li key={`${m.id}-${index}`} className="flex items-start gap-3">
                      ...
                      >
                        {m.role === "assistant" ? (
                          ...
                          >
                            {(() => {
                              // Jika m.content adalah string JSON dengan { output: ... }, ambil output-nya
                              try {
